...
| App load | Fresh Shopify Admin tab showed Dashboard and Orders ready; opened order #1010 and Advanced | Records page loaded with purchased #1001 |
...
| Record files | Order #1010 exported a valid one-page PDF and parseable JSON | Purchased #1001 exported a valid one-page PDF and parseable JSON |
...
1. **Authenticated full walkthrough:** Check all Settings tabs, Billing plan change/cancel/re-approve, Help, and fresh install/reinstall on the final build for each app. Prior rounds exercised install/reinstall and approved test charges, but those actions were not repeated after this deploy. Do not mark them as newly verified.
...
6. **Performance for review:** Both Cloud Run services currently use their ordinary scaling configuration. Set a minimum instance for the review window if first-load latency is unacceptable, then verify it remains enabled during review.
