...
No separate login or credentials are needed. In the `ink-review` development store, open **Ink.** from Shopify Admin › Apps. Orders #1001–#1004 provide test data. Open an order, inspect its activity, then choose **Buy the record ($29 USD)** on an eligible order. Shopify presents a **$29 one-time test charge** on this development store; after approval, return to Records and download the available PDF and JSON files. Settings shows access and privacy requests. On a fresh empty development store, create and fulfill a test order with tracking in Shopify Admin, then open it in Ink. A declined charge should leave the purchase action available; it should not create a purchased record.
...
| Ink. | Orders with an eligible order; the Shopify approval and post-approval return; Records downloads; Dashboard or Settings as optional extra frames | Apps → Dashboard → Orders → Buy the record → Shopify test-charge approval → Records and the two downloads → Settings → Help. Keep the charge screen visibly identified as a development-store test. |
