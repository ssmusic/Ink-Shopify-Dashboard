...
describe("each section of ink has its own address", () => {
  ...
  it("lists Orders in the admin's left nav with its own path, every item on a path of its own", () => {
    ...
    expect(items.map((l) => l.label)).toEqual(["Dashboard", "Orders", "Records", "Settings"]);
    expect(items.map((l) => l.href)).toEqual(["/app/ink/dashboard", "/app/ink/orders", "/app/ink/records", "/app/ink/settings"]);
