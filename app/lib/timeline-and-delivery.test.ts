...
import { carrierNamed, carrierSaid, formatHours, funnel, timeInTransit, whileTheyWaited, type DeliveryRow } from "./delivery-insights";
import { deliveryWindow, kmOrM, lifecycle, openResult, openSentence } from "./order-timeline";
...
describe("the delivery dashboard", () => {
  const rows: DeliveryRow[] = [
    { enrolled_at: iso(0), delivered_at: iso(20), tap_count: 3, gps_verdict: "PASS", location_source: "gps", verified_at_door: true, last_tracking_status: "DELIVERED", carrier_name: "USPS" },
    { enrolled_at: iso(0), delivered_at: iso(30), tap_count: 1, gps_verdict: "FLAGGED", location_source: "GPS", verified_at_door: false, last_tracking_status: "DELIVERED", carrier_name: "UPS" },
    { enrolled_at: iso(0), delivered_at: iso(100), tap_count: 0, gps_verdict: null, last_tracking_status: "DELIVERED" },
    // An NFC-era verdict with no source: opened, but no location shared.
    { enrolled_at: iso(0), delivered_at: iso(26), tap_count: 2, gps_verdict: "pass", location_source: null, last_tracking_status: "DELIVERED" },
    { enrolled_at: iso(0), delivered_at: null, tap_count: 2, gps_verdict: "NOT_SHARED", last_tracking_status: "TRANSIT", carrier_name: "—" },
  ...
  it("buckets time in transit with the console's buckets, skipping a missing or negative pair", () => {
    ...
    expect(t.buckets).toEqual([
      { label: "under 1 d", count: 1 },
      { label: "1–2 d", count: 2 },
      { label: "2–4 d", count: 0 },
      { label: "4–7 d", count: 1 },
      { label: "over 7 d", count: 0 },
    ]);
  ...
  it("says what the carrier said last, largest first, an empty word as 'No carrier update'", () => {
    expect(carrierSaid(rows)).toEqual([
      { status: "DELIVERED", count: 4, ofEnrolledPct: 66.7 },
      { status: "No carrier update", count: 1, ofEnrolledPct: 16.7 },
...
describe("one order's timeline", () => {
  ...
  it("draws the delivery window from the delivered scan to its end, with the first open where it fell", () => {
    const w = deliveryWindow({ delivered_at: iso(20), interaction_window_end: iso(120), first_tap_at: iso(45), within_expected_window: true });
    expect(w).toEqual({ deliveredAt: iso(20), windowEnd: iso(120), firstOpenAt: iso(45), hoursToOpen: 25, openPositionPct: 25, withinExpectedWindow: true });
    // No window end → enrolment + 7 days (the console's fallback); an open before delivery pins to the start.
    const w2 = deliveryWindow({ delivered_at: iso(20), enrolled_at: iso(0), first_tap_at: iso(10) });
    expect(w2?.windowEnd).toBe(iso(168));
    expect(w2?.hoursToOpen).toBe(-10);
    expect(w2?.openPositionPct).toBe(0);
    expect(deliveryWindow({ delivered_at: null, interaction_window_end: iso(120) })).toBeNull();
  });
});
