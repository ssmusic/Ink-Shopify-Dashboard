...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const [doors, purchases] = await Promise.all([
    Promise.all(
      recentOrders.map((o) =>
        inkDoor(admin, session.shop, apiKey, o.proofId, keys).catch(() => ({
          record: null,
          offerLine: null,
          pending: false,
    ...
    // form (services/record-charges.server.ts) — only once it is bought.
  ...
  const purchaseOf = (proofId: string | null) => (proofId ? recordDoorFor(purchases, proofId).purchase : null);
  // A bought record's packet, read with the purchase's own key.
  const packets = await Promise.all(
    recentOrders.map((o) => {
      const url = purchaseOf(o.proofId)?.packet_url;
      return o.proofId && url ? readDisputePacket(o.proofId, url) : Promise.resolve(null);
    }),
  );
  return routeData(
    {
